...
export async function resolvePackage(ctx) {
  ...
  // forked from design-sync lib/source-kit.mjs — sonner.jsx is dead code
  // (App.jsx mounts toaster.jsx's <Toaster/>, not sonner.jsx's) whose own
  // `Toaster` export collides with toaster.jsx's, which ESM `export *`
  // resolves by silently dropping the ambiguous binding to undefined.
  // Dropped here at the source scan rather than editing the live app file.
  const HARDCODED_SRC_EXCLUDE = ['src/components/ui/sonner.jsx'];
  const excludeAbs = new Set(HARDCODED_SRC_EXCLUDE.map((p) => resolve(PKG_DIR, p)));
  const srcFilesAll = srcRoot ? walk(srcRoot, (n) => /\.(tsx|jsx|mdx?)$/.test(n)) : [];
  const srcFiles = srcFilesAll.filter((p) => !excludeAbs.has(resolve(p)));
