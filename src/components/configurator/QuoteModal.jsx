      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }}
        ...
        {submitted ? (
          <div className="text-center py-6">
            ...
            <h3 className="font-serif text-xl text-cream mb-2">We'll be in touch.</h3>
            <p className="text-cream/50 text-sm font-sans max-w-xs mx-auto">Our team will reach out within one business day to arrange your free in-home consultation.</p>
          ...
          <>
            <div className="gold-overline mb-4">FREE CONSULTATION</div>
            <h3 className="font-serif text-2xl text-cream mb-1">Book a Consultation</h3>
            <p className="text-cream/45 text-xs font-sans mb-6">We'll visit your home, review your design, and walk you through the next steps — no obligation.</p>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-[9px] font-sans uppercase tracking-widest text-gold mb-1.5">Phone Number <span className="text-cream/30 normal-case tracking-normal">(optional)</span></label>
                <input type="tel" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
                  placeholder="+1 (000) 000-0000"
                  className="w-full bg-navy border border-cream/20 text-cream placeholder:text-cream/25 px-4 py-2.5 text-sm font-sans focus:border-gold focus:outline-none transition-colors" />
              </div>
              <div>
                <label className="block text-[9px] font-sans uppercase tracking-widest text-gold mb-1.5">Anything we should know? <span className="text-cream/30 normal-case tracking-normal">(optional)</span></label>
                <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
                  placeholder="e.g. We're renovating our kitchen and bathroom together, targeting this fall."
                  rows={3}
              ...
              <div className="bg-navy border border-cream/10 p-4">
                <p className="text-[9px] font-sans uppercase tracking-widest text-gold mb-2">Your Design</p>
              ...
              <button onClick={handleSubmit} disabled={!form.email || !form.name || submitting}
                ...
                {submitting ? 'Sending…' : <>Request My Free Consultation <ArrowRight className="w-3.5 h-3.5" /></>}
              </button>
            </div>
          </>
        )}
